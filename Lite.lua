...
local function iPremiumBtn(parent)
    ...
    lck.Size = UDim2.new(0,13,0,13) 
    lck.Position = UDim2.new(0,8,0.5,-6)
    lck.BackgroundTransparency = 1 
    lck.Image = "rbxassetid://78672912777756"
    lck.ImageColor3 = LOCK_COL 
    lck.BorderSizePixel = 0 
    lck.ZIndex = 6 
    lck.Parent = b
    return b
end
...
local tDivF=makeItem(1) 
iDiv(tDivF) 
table.insert(tDropItems,{f=tDivF,h=1})

local tPremF=makeItem(ITEM_H) 
iPremiumBtn(tPremF) 
table.insert(tDropItems,{f=tPremF,h=ITEM_H})
...
local hDivF=makeItem(1) 
iDiv(hDivF) 
table.insert(hDropItems,{f=hDivF,h=1})

local hPremF=makeItem(ITEM_H) 
iPremiumBtn(hPremF) 
table.insert(hDropItems,{f=hPremF,h=ITEM_H})

local DEV_H = 28
local hjDevF = makeHalfItem(DEV_H)
local hjDevLbl = Instance.new("TextLabel")
hjDevLbl.Size=UDim2.new(1,0,1,0) 
hjDevLbl.BackgroundTransparency=1
hjDevLbl.Text="In Development" 
hjDevLbl.TextColor3=Color3.fromRGB(148,112,0)
hjDevLbl.Font=Enum.Font.GothamBold 
hjDevLbl.TextSize=10
hjDevLbl.TextXAlignment=Enum.TextXAlignment.Center
hjDevLbl.ZIndex=5 
hjDevLbl.Parent=hjDevF
table.insert(hjDropItems,{f=hjDevF,h=DEV_H})

local hfDevF = makeHalfItem(DEV_H)
local hfDevLbl = Instance.new("TextLabel")
hfDevLbl.Size=UDim2.new(1,0,1,0) 
hfDevLbl.BackgroundTransparency=1
hfDevLbl.Text="In Development" 
hfDevLbl.TextColor3=Color3.fromRGB(148,112,0)
hfDevLbl.Font=Enum.Font.GothamBold 
hfDevLbl.TextSize=10
hfDevLbl.TextXAlignment=Enum.TextXAlignment.Center
hfDevLbl.ZIndex=5 
hfDevLbl.Parent=hfDevF
...
local function layoutAll()
    ...
    if trackDropOpen then
        for _,item in ipairs(tDropItems) do
            item.f.Position=UDim2.new(0,5,0,y) 
            item.f.Visible=true 
            y=y+item.h+ITEM_G
        end
    else
    ...
    local hopLY=y+GAP
    hLbl.Position=UDim2.new(0,8,0,hopLY) 
    hWrap.Position=UDim2.new(0,5,0,hopLY+LABEL_H)
    local hopBtm=hopLY+LABEL_H+ROW_H 
    y=hopBtm+ITEM_G
    ...
    hjWrapL.Position=UDim2.new(0,5,0,hjY) 
    hjWrapL.Size=UDim2.new(0.5,-8,0,HEAD_H)
    hfWrapR.Position=UDim2.new(0.5,3,0,hjY) 
    hfWrapR.Size=UDim2.new(0.5,-8,0,HEAD_H)
    ...
    if hjDropOpen then
        ...
        for _,item in ipairs(hjDropItems) do
            item.f.Position=UDim2.new(0,5,0,iy) 
            item.f.Visible=true
            iy=iy+item.h+ITEM_G 
            hjDropH=hjDropH+item.h+ITEM_G
        end
    else
        for _,item in ipairs(hjDropItems) do
            item.f.Visible=false
    ...
    if hfDropOpen then
        ...
        for _,item in ipairs(hfDropItems) do
            item.f.Position=UDim2.new(0.5,3,0,iy) 
            item.f.Visible=true
            iy=iy+item.h+ITEM_G 
            hfDropH=hfDropH+item.h+ITEM_G
        end
    else
        for _,item in ipairs(hfDropItems) do
            item.f.Visible=false
...
local function setTrack(s)
    trackEnabled=s 
    tTog.TextColor3=s and ON_TC or OFF_TC
...
local function setHop(s)
    hopEnabled=s 
    hTog.TextColor3=s and ON_TC or OFF_TC
...
local function setTeam(s)
    teamCheckEnabled=s 
    teamBtn.Text=s and "Team Check: ON" or "Team Check: OFF"
    teamBtn.BackgroundColor3=s and GREEN_BG or RED_BG 
    teamBtn.TextColor3=s and GREEN_TC or RED_TC
end
...
tArrBtn.MouseButton1Click:Connect(function()
    trackDropOpen=not trackDropOpen 
    tArrImg.Image=trackDropOpen and ARROW_UP or ARROW_DOWN 
    layoutAll()
end)

hArrBtn.MouseButton1Click:Connect(function()
    hopDropOpen=not hopDropOpen 
    hArrImg.Image=hopDropOpen and ARROW_UP or ARROW_DOWN 
    layoutAll()
end)

tTog.MouseButton1Click:Connect(function()
    setTrack(not trackEnabled)
end)

hTog.MouseButton1Click:Connect(function()
    setHop(not hopEnabled)
end)
...
teamBtn.MouseButton1Click:Connect(function()
    setTeam(not teamCheckEnabled)
end)

local hjArrOpen,hfArrOpen = false,false

hjTog.MouseButton1Click:Connect(function()
    hjDropOpen=not hjDropOpen 
    layoutAll()
end)

hfTog.MouseButton1Click:Connect(function()
    hfDropOpen=not hfDropOpen 
    layoutAll()
end)
...
minBtn.MouseButton1Click:Connect(function()
    ...
    if minimized then
        ...
        hjDropOpen=false 
        hfDropOpen=false
        tArrImg.Image=ARROW_DOWN 
        hArrImg.Image=ARROW_DOWN
        sep.Visible=false 
        tWrap.Visible=false 
        tLbl.Visible=false 
        hWrap.Visible=false 
        hLbl.Visible=false
        hjWrapL.Visible=false 
        hfWrapR.Visible=false 
        hdLbl.Visible=false 
        vDivLine.Visible=false
        ...
        hWrap.Visible=true 
        hLbl.Visible=true
        hjWrapL.Visible=true 
        hfWrapR.Visible=true 
        hdLbl.Visible=true 
        vDivLine.Visible=true
        tLbl.Position=UDim2.new(0,8,0,TRACK_LABEL_Y) 
        tWrap.Position=UDim2.new(0,5,0,TRACK_ROW_Y)
        layoutAll()
    end
...
local function nearWall(hrp)
    ...
    for i=1,4 do
        ...
        if r then
            ...
            if not h:IsA("TrussPart") and not n:find("ladder") and not n:find("truss") and not n:find("climb") then
                return true
            end
        end
    end 
...
local function doFlick(rw)
    ...
    if rw and not nearWall(hrp) then 
        debounce=false 
        return 
    end
...
local function blockedByGlass(from, to, ignoreList)
    ...
    for _=1,6 do
        local result=workspace:Raycast(from,dir.Unit*dist,params) 
...
local function isGreen(other)
    ...
    local function g(c) 
        return c.G>0.4 and c.G>c.R*1.5 and c.G>c.B*1.5 
    end
