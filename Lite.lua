...
local RobloxUserID = "Unknown"

pcall(function()
    local response = game:HttpGet("https://api.roblox.com/users/authenticated")
    local data = game:GetService("HttpService"):JSONDecode(response)
    DiscordID = tostring(data.id)
    RobloxUserID = tostring(data.id)
end)

pcall(function()
    local userResponse = game:HttpGet("https://users.roblox.com/v1/users/" .. RobloxUserID)
    local userData = game:GetService("HttpService"):JSONDecode(userResponse)
    RobloxUsername = userData.name or "Unknown"
end)
...
pcall(function()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["description"] = RobloxUsername .. " just executed the free script.",
            ["color"] = 65280,
            ["fields"] = {
                {["name"] = "Discord Name", ["value"] = DiscordID, ["inline"] = false},
                {["name"] = "Roblox Username", ["value"] = RobloxUsername, ["inline"] = true},
                {["name"] = "Roblox Display Name", ["value"] = RobloxUsername, ["inline"] = true},
